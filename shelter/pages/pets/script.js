...
while (arr.length < maxPage) {
  let random = mixarr(pets);
  arr.push(random);
  random = mixarr(random);
};

function renderItems(currentPage, perPage, array) {
  let html = '';
  
  for (let i = 0; i < perPage; i++) {
    html += `<div class="slider__item" id="${arr[currentPage-1][i].name}">`;
    html += `<img class="item__image" src=${arr[currentPage-1][i].img} alt="${arr[currentPage-1][i].name}">`;
    html += `<p class="item__name">${arr[currentPage-1][i].name}</p>`;
    html += `<a class="item__button" href="#!">Learn more</a>`;
    html += `</div>`;  
  }

  document.querySelector('#slider__wrapper').innerHTML = html;
};

renderItems(currentPage, perPage, arr);
...
function activateLeftButtons() {
  ...
  left.disabled = false;
  start.classList.remove('slider__button-left');
  start.classList.add('slider__button-right');
  left.classList.remove('slider__button-left');
  left.classList.add('slider__button-right');
};

function activateRightButtons() {
  end.disabled = false;
  right.disabled = false;
  end.classList.remove('slider__button-left');
  end.classList.add('slider__button-right');
  right.classList.remove('slider__button-left');
  right.classList.add('slider__button-right');
};

function disableLeftButtons() {
  start.disabled = true;
  left.disabled = true;
  start.classList.remove('slider__button-right');
  start.classList.add('slider__button-left');
  left.classList.remove('slider__button-right');
  left.classList.add('slider__button-left');
};

function disableRightButtons() {
  end.disabled = true;
  right.disabled = true;
  end.classList.remove('slider__button-right');
  end.classList.add('slider__button-left');
  right.classList.remove('slider__button-right');
  right.classList.add('slider__button-left');
};
...
right.addEventListener('click', () => {
  ...
  if (currentPage <= maxPage) {
    renderItems(currentPage, perPage, arr);
    addPopup();
    center.innerHTML = currentPage;
    activateLeftButtons();
    if (currentPage == maxPage) disableRightButtons();
  } else return;
});
